...
- **on-failure**: 컨테이너가 **비정상 종료(에러 코드 반환)된 경우에만 재시작**한다.
- **always**: 컨테이너가 중지될 경우 **항상 재시작**하며, 시스템 재부팅 시에도 자동으로 재시작된다.
- **unless-stopped**: **수동으로 중지되지 않은 경우 컨테이너를 항상 재시작**하며, 시스템 재부팅 시에도 동일하게 동작한다.
