...
<font color="#00b050">함수의 연속</font>이란 <font color="#ffff00">그래프가 끊어지지 않고 계속 연결된 함수</font>를 의미합니다. 그래프가 계속 연결된다는 의미를 이해하기 위해 함수가 연속되지 않는 불연속 개념부터 먼저 살펴봅시다.

함수의 불연속은 $x=a$ 에서 끊어진 함수로, 다음 그래프를 살펴봅시다.

![[Pasted image 20240229160148.png]]

그림의 세 개의 그래프 모두 불연속적(끊어진) 그래프입니다.
- 첫번째 그래프는 함수 $f(x)$ 가 $x=a$ 에서 정의되어 있지 않기에
- 두번쨰 그래프는 우극한의 갑싱 다르기에
- 세번째 그래프는 $\lim_{x \rightarrow a}f(x)$ 와 $f(a)$ 가 다르기에
불연속 그래프입니다. 이 세 가지 불연속 경우들을 반대로 생각하면 함수의 연속이 되는 조건들을 쉽게 도출할 수 있습니다.

---

$f(x)$ 가 $x = a$ 에서 정의되어 있지 않다는 표현은 어떤 의미인가요?

아래 그림과 같이 두함수 $f(x) = \frac{x^2-1}{x-1}$ , $g(x) = x+1$ 이 있다고 합시다. 두 함수를 그래프로 표현하고자 $f(x)$ 에 대한 함수를 간단히 정리해 봅니다.

함수 $f(x)$ 에 대한 그래프 표현을 위해 수식을 간단히 하면, $f(x) = \frac{(x+1)(x-1)}{x-1} = x+1$ 이 됩니다. 단 $x$ 가 1일 떄는 분모가 1이 되면서 $f(x)$ 값이 0이 되므로 $x \ne 1$ 이어야 합니다. 이때 $x \ne 1$ 은 함수 $f(x)$ 에서 $x = 1$ 일 때 함수가 정의되지 않았다는 의미이며, 그래프상에서는 $x = 1$ 에서 구멍이 비어 있는 동그라미로 표현합니다. 참고로 $x = 1$ 에서 값이 정의되어 있다면 구멍이 채워져 있는 동그라미로 표현합니다.

![[Pasted image 20240229161621.png]]

$g(x)$ 처럼 $x = 1$ 인 지점에서 함수의 그래프가 끊어지지 않고 이어지는 것을 **x = 1에서 연속이다** 라고 표현하며, $f(x)$ 처럼 $x = 1$ 에서 함수 그래프가 끊어지며 **x = 1에서 불연속이다** 라고 표현합니다.

---

다음은 <font color="#ffff00">함수가 연속이 되는 조건</font>들입니다. 

1. 함수 $f(x)$ 가 $x=a$ 에서 정의되어야 합니다.
2. $\lim_{x \rightarrow a}f(x)$ 가 존재해야합니다.
3. $\lim_{x \rightarrow a}f(x) = f(a)$ 입니다.

즉, 그래프 형태가 아래 그림과 같아야합니다.

![[Pasted image 20240229161922.png|512]]

정리하면 함수의 연속성을 만족하려면 다음 수식을 만족해야 합니다.
